...
type Router struct {
	store    store.ConfigStore // may be nil when no DB configured
...
func (r *Router) hasStore() bool { return r.store != nil }
...
func registerRoutes(mux *http.ServeMux, r *Router) {
	// Store-dependent routes (only available when DB is configured).
	if r.hasStore() {
		// Provider endpoints
	...
	// Store-independent routes (always available).
	...
	mux.HandleFunc("POST /v1/admin/mode", r.handlePostMode)
}
